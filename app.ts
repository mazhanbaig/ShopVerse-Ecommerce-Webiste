...
window.addEventListener("DOMContentLoaded", () => {
  ...
  function showProduct(index: number) {
    ...
    else if (index >= products.length) currentIndex = 0;
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  let storedProducts: ProductWithCategory[];
  try {
  ...
  const categoryToContainerId: Record<string, string> = {
  ...
  function createSliderContainer(): HTMLDivElement {
  ...
  function createProductCard(product: ProductWithCategory): HTMLDivElement {
    const card = document.createElement("div");
    card.className = `
      inline-block
      bg-gray-50
    ...
    card.innerHTML = `
      ...
      <img src="${product.image || product.imageUrl}" alt="${product.name}" style="width: 100%; height: 100%; object-fit: contain; padding: 1rem;" />
    </div>
